...
class TestGitHubWebhook:
        ...
        with patch.dict("os.environ", {"GITHUB_WEBHOOK_SECRET": "test_secret"}):
            ...
            with patch("app.GITHUB_WEBHOOK_SECRET", "test_secret"):
                result = verify_github_signature(
                    payload.encode("utf-8"), wrong_signature
                )
    ...
    def test_webhook_issue_event_success(
        self, mock_kafka, client, sample_issue_payload
    ):
        ...
        mock_future.get.return_value = MagicMock(
            topic="issues.raw", partition=0, offset=123
        )
    ...
    @patch("app.get_kafka_producer")
    @patch("app.GITHUB_WEBHOOK_SECRET", "test_secret")
    def test_webhook_comment_event_success(
        self, mock_kafka, client, sample_comment_payload
    ):
        ...
        mock_future = MagicMock()
        mock_future.get.return_value = MagicMock(
            topic="issues.raw", partition=0, offset=124
        )
        mock_producer.send.return_value = mock_future
        mock_kafka.return_value = mock_producer
...
class TestRateLimiting:
    ...
    def test_rate_limit_enforcement(self, client):
        ...
        active_requests = [
            timestamp
            for timestamp in middleware.requests[client_ip]
